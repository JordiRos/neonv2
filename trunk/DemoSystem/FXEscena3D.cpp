...
//
//---------------------------------------------------------------------------//
void CFXEscena3D::Draw(CDisplayDevice *pDD)
{
  ...
  else
  {
    ...
    if (m_bMotionBlur)
      CEffects::DoMotionBlurPass1(pDD, m_RenderTarget.iTexture);
  ...
  if (m_bMotionBlur)
    CEffects::DoMotionBlurPass2(pDD, m_RenderTarget.iTexture, m_fMotionIntens * 0.4f + 0.55f);
}
