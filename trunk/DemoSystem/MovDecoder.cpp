...
void MovDecoder_EndSubsystem()
{
  CMovDecoder::EndSubsystem();
}
...
//
//---------------------------------------------------------------------------//
void CMovDecoder::InitSubsystem()
{
	OSErr anErr = noErr;
  InitializeQTML();
  ...
  */
  ...
  if (noErr != Gestalt (gestaltQuickTime, &qtVersion))
	{
		GLOG(("WAR: QuickTime extension is not present in this system"));
    return;
	}
	if( (qtVersion >> 16 ) < 0x400 ) 
	{
		GLOG(("WAR: QT 4.0 or higher needed to play .MOV movies"));
    return;
	}
	anErr = EnterMovies();
	if(anErr != noErr) 
	{
		GLOG(("ERR: Problems initializing QT::Entermovies()"));
    return;
	}
  */
}
...
//
//---------------------------------------------------------------------------//
bool CMovDecoder::Init(const char *pszFilename, unsigned uBpp)
{
...
//
//---------------------------------------------------------------------------//
int CMovDecoder::TimeToFrame(float fTime)
{
  return 0;
  //return MovStreamTimeToSample(m_Mov, int(fTime*1000.0f));
}
...
//
//---------------------------------------------------------------------------//
float CMovDecoder::Length()
{
  /*
  int iFrames = MovStreamLength(m_Mov);
  int iTime   = MovStreamSampleToTime(m_Mov, iFrames);
...
//
//---------------------------------------------------------------------------//
bool CMovDecoder::GetFrame(int iFrame, CTextura *pTextura)
{
  return false;
}
